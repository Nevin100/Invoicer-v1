...
export async function POST(
  ...
  try {
    ...
    if (pd?.qrCode) {
      ...
      const upiString = `upi://pay?pa=${pd.upiId}&pn=${encodeURIComponent(businessName)}&am=${invoice.totalAmount}&cu=INR`;
      upiQrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&data=${encodeURIComponent(upiString)}&margin=10`;
    }

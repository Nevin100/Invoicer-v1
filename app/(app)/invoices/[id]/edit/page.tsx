...
function InvoicePreview({
  ...
  // Generate UPI QR URL
  const upiQrUrl = pd?.qrCode
    ? pd.qrCode
    : pd?.upiId
    ? `https://api.qrserver.com/v1/create-qr-code/?size=120x120&data=${encodeURIComponent(`upi://pay?pa=${pd.upiId}&pn=${encodeURIComponent(businessName)}&am=${invoice.totalAmount}&cu=INR`)}&margin=8`
    : null;

  return (
    <div className="w-full bg-white shadow-2xl" style={{ fontFamily: "'DM Sans', sans-serif", minHeight: "842px" }}>
      ...
      <div style={{ background: accent.accent, padding: "36px 48px 32px" }}>
        <div className="flex items-start justify-between">
          <div>
            ...
            <div className="text-sm mt-1" style={{ color: "rgba(255,255,255,0.6)" }}>{businessEmail}</div>
          </div>
          <div className="text-right">
            <div className="text-white text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: "rgba(255,255,255,0.5)" }}>Invoice</div>
            <div className="text-white font-bold text-xl">#{invoice.invoiceNumber}</div>
            <div className="text-xs mt-2 px-3 py-1 rounded-full inline-block font-semibold" style={{ background: "rgba(255,255,255,0.15)", color: "white" }}>
              {invoice.status}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-0" style={{ borderBottom: "1px solid #f0f0f0" }}>
        {[
          ...
          <div key={i} className="px-8 py-5" style={{ borderRight: i < 2 ? "1px solid #f0f0f0" : "none", background: cell.highlight ? accent.light : "white" }}>
            <div className="text-xs font-semibold uppercase tracking-widest mb-1" style={{ color: "#999" }}>{cell.label}</div>
            <div className="font-semibold text-sm" style={{ color: accent.text }}>{cell.value}</div>
          ...
          <thead>
            <tr style={{ borderBottom: `2px solid ${accent.accent}` }}>
              {["Item", "Qty", "Rate", "Amount"].map((h, i) => (
                <th key={h} className="pb-3 text-xs font-semibold uppercase tracking-widest"
                  style={{ color: accent.accent, textAlign: i === 0 ? "left" : "right" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {invoice.items?.map((item, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #f5f5f5" }}>
                <td className="py-3.5">
                  ...
                  {item.description && <div className="text-xs mt-0.5" style={{ color: "#aaa" }}>{item.description}</div>}
                </td>
      ...
      <div className="px-8 pb-6">
        <div className="ml-auto" style={{ maxWidth: "260px" }}>
          ...
          <div className="flex justify-between items-center mt-3 pt-3 rounded-lg px-4 py-3" style={{ background: accent.accent }}>
      ...
      {(showUpi || showBank) && (
        <div className="mx-8 mb-6 rounded-xl overflow-hidden" style={{ border: `1px solid ${accent.accent}22` }}>
          <div className="px-5 py-3 text-xs font-bold uppercase tracking-widest" style={{ background: accent.accent + "12", color: accent.accent }}>
            Payment Details
          </div>
          <div className="p-5 flex flex-wrap gap-6" style={{ background: "#fafafa" }}>

            {/* UPI */}
            {showUpi && (
              <div className="flex items-start gap-4">
                {upiQrUrl && (
                  <img src={upiQrUrl} alt="UPI QR" className="w-20 h-20 rounded-lg border" style={{ borderColor: "#e5e7eb" }} />
                )}
                <div className="space-y-1">
                  <p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: "#aaa" }}>UPI</p>
                  <p className="text-sm font-bold" style={{ color: "#1a1a1a" }}>{pd?.upiId}</p>
                  <p className="text-xs" style={{ color: "#999" }}>Scan QR or pay directly to UPI ID</p>
                </div>
              </div>
            )}
            ...
            {showBank && (
              <div className="space-y-1.5">
                <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: "#aaa" }}>Bank Transfer</p>
                {[
                  ...
                  <div key={row.label} className="flex gap-3 text-xs">
                    <span className="w-24 font-semibold" style={{ color: "#aaa" }}>{row.label}</span>
                    <span className="font-bold" style={{ color: "#1a1a1a" }}>{row.value}</span>
                  </div>
                ))}
              </div>
            )}
...
export default function InvoiceEditPage() {
  ...
  const saveCustomization = async () => {
    await fetch(`/api/invoices/${id}`, {
      ...
      body: JSON.stringify({ logo, accentColor: selectedTheme.accent }),
    });
  };
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error === "insufficient_credits" ? "Not enough credits to send this invoice." : data.error || "Failed to send invoice");
        return;
      }
      setSent(true);
    ...
    <div className="min-h-screen" style={{ background: "#0a0a0a" }}>
      ...
      <div className="sticky top-0 z-50 flex items-center justify-between px-6 py-3"
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => router.back()} className="text-white/40 hover:text-white/80 transition-colors text-sm flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-3">
          <button onClick={saveCustomization} className="text-sm px-4 py-1.5 rounded-lg transition-all"
            style={{ background: "rgba(255,255,255,0.06)", color: "rgba(255,255,255,0.6)", border: "1px solid rgba(255,255,255,0.08)" }}>
            Save Draft
          </button>
          <motion.button onClick={handleSend} disabled={sending || sent} whileTap={{ scale: 0.97 }}
            ...
            {sending ? (
              <><div className="w-3.5 h-3.5 border border-white/40 border-t-white rounded-full animate-spin" />Sending...</>
            ) : sent ? (
              <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M20 6L9 17l-5-5" /></svg>Sent!</>
            ) : (
              <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z" /></svg>Send Invoice</>
            )}
          </motion.button>
        </div>
      </div>
        ...
        {sent && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 flex items-center justify-center"
            ...
            <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}
              ...
              <div className="w-20 h-20 rounded-full flex items-center justify-center" style={{ background: selectedTheme.accent }}>
              ...
              <div>
                ...
                <div className="text-white/40 text-sm">Email delivered to {invoice.client?.email}</div>
      ...
      <div className="flex h-[calc(100vh-49px)]">
        ...
        {/* LEFT PANEL */}
        <div className="w-72 flex-shrink-0 overflow-y-auto" style={{ background: "#111", borderRight: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="p-5 space-y-6">

            {/* Logo */}
            <div>
              <label className="text-xs font-semibold uppercase tracking-widest mb-3 block" style={{ color: "rgba(255,255,255,0.3)" }}>Business Logo</label>
              <div onClick={() => fileInputRef.current?.click()} className="relative rounded-xl overflow-hidden cursor-pointer transition-all group"
                style={{ border: "1.5px dashed rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.03)", minHeight: "96px" }}>
                {logo ? (
              ...
              {logo && (
                <button onClick={() => setLogo(null)} className="text-xs mt-2 transition-colors" style={{ color: "rgba(255,255,255,0.25)" }}
            ...
            {/* Business Info */}
            <div>
              <label className="text-xs font-semibold uppercase tracking-widest mb-3 block" style={{ color: "rgba(255,255,255,0.3)" }}>Your Business</label>
              <div className="space-y-2">
                ...
                <input type="email" placeholder="your@email.com" value={businessEmail} onChange={e => setBusinessEmail(e.target.value)}
                  className="w-full text-sm px-3 py-2.5 rounded-lg outline-none transition-all"
                  style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.8)" }} />
              </div>
            </div>

            {/* ✅ Payment Details status in panel */}
            <div className="rounded-xl p-4" style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="text-xs font-semibold uppercase tracking-widest mb-3" style={{ color: "rgba(255,255,255,0.25)" }}>Payment Details</div>
              {invoice.paymentDetails?.upiId || invoice.paymentDetails?.accountNo ? (
                <div className="space-y-1.5">
                  {invoice.paymentDetails?.upiId && (
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                      <span className="text-xs" style={{ color: "rgba(255,255,255,0.5)" }}>UPI: {invoice.paymentDetails.upiId}</span>
                    </div>
                  )}
                  {invoice.paymentDetails?.accountNo && (
                    <div className="flex items-center gap-2">
                      <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
                      <span className="text-xs" style={{ color: "rgba(255,255,255,0.5)" }}>Bank: {invoice.paymentDetails.bankName || "Account configured"}</span>
                    </div>
                  )}
                </div>
              ) : (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs" style={{ color: "rgba(255,255,255,0.3)" }}>Not configured — add from profile</span>
                </div>
              )}
            </div>
            ...
            {/* Theme */}
            <div>
              <label className="text-xs font-semibold uppercase tracking-widest mb-3 block" style={{ color: "rgba(255,255,255,0.3)" }}>Accent Color</label>
              <div className="grid grid-cols-3 gap-2">
                {THEMES.map(theme => (
                  <button key={theme.name} onClick={() => setSelectedTheme(theme)}
                    ...
                    style={{ background: theme.accent, color: "white", outline: selectedTheme.name === theme.name ? "2px solid white" : "2px solid transparent", outlineOffset: "2px" }}>
                    {theme.name}
                    {selectedTheme.name === theme.name && <span className="absolute top-1 right-1 w-1.5 h-1.5 bg-white rounded-full" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.25)" }}>Summary</div>
              {[
            ...
            <div className="rounded-xl p-4 space-y-3" style={{ background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(255,255,255,0.07)" }}>
              <div className="text-xs font-semibold uppercase tracking-widest" style={{ color: "rgba(255,255,255,0.2)" }}>On Send</div>
              {[
                "Razorpay payment link created",
                "UPI/Bank QR embedded in email",
                ...
                <div key={i} className="flex items-start gap-2.5">
                  <div className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: selectedTheme.accent + "33" }}>
                    <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke={selectedTheme.accent} strokeWidth="3"><path d="M20 6L9 17l-5-5" /></svg>
                  </div>
        ...
        <div className="flex-1 overflow-y-auto p-8" style={{ background: "#0d0d0d" }}>
          <div className="max-w-2xl mx-auto">
            ...
            <motion.div key={selectedTheme.name} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
              className="rounded-xl overflow-hidden shadow-2xl" style={{ boxShadow: "0 24px 80px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.05)" }}>
              <InvoicePreview invoice={invoice} logo={logo} accent={selectedTheme} businessName={businessName} businessEmail={businessEmail} />
            </motion.div>

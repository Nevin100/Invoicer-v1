          ...
          <CardContent className="p-10 md:p-14">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-12">
              ...
              <FormField
                ...
                value={client.postal}
                onChange={(e: any) => handleChange("postal", e.target.value)}
              />
              ...
              <FormField
                ...
                onChange={(e: any) => handleChange("serviceCharge", Number(e.target.value))}
              />

              <div className="sm:col-span-2">
                <FormField
                  id="website"
                  label="Digital Presence / Website"
            ...
            <div className="flex flex-col sm:flex-row justify-end items-center gap-10 mt-20 pt-10 border-t-2 border-slate-50">
              <button
                type="button"
                className="text-slate-500 font-black text-sm uppercase tracking-widest hover:text-slate-900 transition-colors cursor-pointer"
              ...
              <Button
                className="bg-slate-900 hover:bg-indigo-600 text-white px-14 h-16 rounded-[1.25rem] font-black text-base uppercase tracking-widest shadow-2xl shadow-slate-200 transition-all active:scale-95 disabled:opacity-50 cursor-pointer group"
                disabled={loading}
...
function FormField({ id, label, icon, value, onChange, type = "text" }: any) {
  return (
    <div className="space-y-4 group">
      <Label htmlFor={id} className="text-xs font-black uppercase tracking-widest text-slate-600 ml-1 group-focus-within:text-indigo-600 transition-colors">
        {label}
      </Label>
      <div className="flex items-center gap-4 bg-[#fdfdfb] border-2 border-slate-200 group-focus-within:border-indigo-600 group-focus-within:ring-4 group-focus-within:ring-indigo-50/50 group-focus-within:bg-white rounded-[1.25rem] px-6 py-1 transition-all shadow-sm group-hover:border-slate-300">
        <span className="opacity-70 group-focus-within:opacity-100 transition-all scale-110">
          {icon}
        </span>
        <Input
          id={id}
          ...
          className="border-none bg-transparent shadow-none focus-visible:ring-0 p-0 h-14 text-slate-900 font-bold text-lg placeholder:text-slate-300 placeholder:font-semibold"
          value={value}
          onChange={onChange}
        />
      </div>

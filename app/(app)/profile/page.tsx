...
const EditModal = ({
  ...
  profile: Profile;
  onClose: () => void;
  onSave: (pageNumber: number, data: any) => Promise<void>;
}) => {
  ...
  const inputClass =
    "w-full px-4 h-12 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-400 outline-none transition-all";
      ...
      >
        ...
        >
          <div className="p-8 border-b border-slate-50 flex justify-between items-center">
            ...
            <button
              onClick={onClose}
              className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors"
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div className="p-8 overflow-y-auto space-y-6">
            ...
            {pageNumber === 3 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <InputWrapper label="Revenue Range">
                  <input
                    className={inputClass}
                    value={data.monthlyRevenueRange}
                    onChange={(e) =>
                      setData({ ...data, monthlyRevenueRange: e.target.value })
                    }
                  />
                </InputWrapper>

                <InputWrapper label="Average Deal Size">
                  <input
                    className={inputClass}
                    value={data.avgDealSize}
                    onChange={(e) =>
                      setData({ ...data, avgDealSize: e.target.value })
                    }
                  />
                </InputWrapper>
                ...
                <InputWrapper label="Payment Terms">
                  <input
                    className={inputClass}
                    value={data.paymentTermsPreference}
                    onChange={(e) =>
                      setData({
                        ...data,
                        paymentTermsPreference: e.target.value,
                      })
                    }
                  />
                </InputWrapper>

                <InputWrapper label="Daily Business Scale">
                  <input
                    className={inputClass}
                    value={data.dailyBusinessScale}
                    onChange={(e) =>
                      setData({ ...data, dailyBusinessScale: e.target.value })
                    }
                  />
                </InputWrapper>
              </div>
                ...
                <InputWrapper label="Field">
                  <input
                    ...
                    onChange={(e) =>
                      setData({
                        ...data,
                        education: { ...data.education, field: e.target.value },
                      })
                    }
                  />
                </InputWrapper>

                <InputWrapper label="Institution">
                  <input
                    className={inputClass}
            ...
            <div className="bg-amber-50 rounded-2xl p-4 border border-amber-100">
              ...
              <p className="text-xs text-amber-600/80 font-medium">
                Updating this will reflect immediately on your public dashboard.
              </p>
            </div>
          </div>
          ...
          <div className="p-8 border-t border-slate-50 flex gap-4">
            <button
              onClick={onClose}
              className="flex-1 h-14 rounded-2xl font-black uppercase tracking-widest text-xs text-slate-400 hover:bg-slate-50 transition-colors"
            >
              Cancel
            </button>
            <button
            ...
            <div className="relative flex flex-col md:flex-row items-end gap-6 -mt-12 md:-mt-16">
              ...
              <div className="flex-1 mb-2">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {profile.isComplete && (
                    <BadgeCheck className="text-indigo-500" size={28} />
                  )}
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            <div className="space-y-1">
              <InfoItem
                label="Entity"
                value={profile.page2.companyName}
                icon={Briefcase}
              />
              <InfoItem
                label="Industry"
                value={profile.page2.industry}
                icon={Target}
              />
              <InfoItem
                label="Team Size"
                value={TEAM_LABELS[profile.page2.teamSize]}
                icon={Users}
              />
              <InfoItem
                label="GSTIN"
                value={profile.page2.gstin}
                icon={FileText}
              />
            </div>
          </SectionCard>
          ...
          >
            <div className="space-y-1">
              <InfoItem
                label="Revenue"
                value={REVENUE_LABELS[profile.page3.monthlyRevenueRange]}
                icon={IndianRupee}
              />
              <InfoItem
                label="Deal Size"
                value={DEAL_LABELS[profile.page3.avgDealSize]}
                icon={Zap}
              />
              <InfoItem
                label="Payments"
                value={profile.page3.paymentTermsPreference}
                icon={Calendar}
              />
              <div className="mt-4 flex flex-wrap gap-2">
                {profile.page3.primaryServices.slice(0, 3).map((s) => (
                  <span
                    key={s}
                    className="px-3 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-black rounded-lg uppercase tracking-wider"
                  >
                    {s}
                  </span>
                ))}
          ...
          <SectionCard
            ...
            onEdit={() => setEditingPage(4)}
          >
            <div className="space-y-1">
              <InfoItem
                label="Education"
                value={`${profile.page4.education.degree} in ${profile.page4.education.field}`}
                icon={Award}
              />
              <InfoItem
                label="Institution"
                value={profile.page4.education.institution}
                icon={Building2}
              />
              <div className="mt-6">
                <h5 className="text-[9px] font-black uppercase text-slate-400 mb-2">
                  Key Hobbies
                </h5>
                <div className="flex gap-2">
                  {profile.page4.hobbies.map((h) => (
                    <div
                      key={h}
                      className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-all cursor-help"
                      title={h}
                    >
                      <Heart size={16} />
                    </div>

...
import {
  User, Building2, TrendingUp, GraduationCap, CreditCard,
  ChevronRight, ChevronLeft, Check, Loader2,
  Briefcase, MapPin, Globe, Phone, FileText,
  Target, Zap, Star, Award, QrCode, Landmark,
} from "lucide-react";
...
interface Page1Data {
  ...
  userType: "freelancer" | "business_owner" | ""; occupation: string; tagline: string;
}
interface Page2Data {
  companyName: string; companyType: string; industry: string; yearsInBusiness: string;
  teamSize: string; officeAddress: { street: string; city: string; state: string; pincode: string };
...
interface Page4Data {
  ...
  achievements: string[]; hobbies: string[]; pastTradeHighlights: string; currentTargets: string;
}
interface Page5Data {
  preferredMethod: "upi" | "bank" | "both";
  upiId: string;
  accountName: string; accountNo: string; ifsc: string; bankName: string; branchName: string;
  qrCode: string;
}
...
const STEPS = [
  ...
  { id: 5, label: "Payments",   icon: CreditCard,  color: "from-violet-500 to-purple-600" },
];
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <Field label="Full Name" icon={User}><input className={inputCls} placeholder="Nevin Bali" value={data.fullName} onChange={e => set("fullName", e.target.value)} /></Field>
        <Field label="Phone" icon={Phone}><input className={inputCls} placeholder="+91 98765 43210" value={data.phone} onChange={e => set("phone", e.target.value)} /></Field>
      </div>
      <Field label="Occupation / Role" icon={Briefcase}><input className={inputCls} placeholder="e.g. Full Stack Developer" value={data.occupation} onChange={e => set("occupation", e.target.value)} /></Field>
      <Field label="Your Tagline" icon={Zap}><input className={inputCls} placeholder="e.g. Building products that matter" value={data.tagline} onChange={e => set("tagline", e.target.value)} /></Field>
      <Field label="Short Bio" icon={FileText}>
        <textarea className={textareaCls} rows={3} placeholder="Tell us a bit about yourself..." value={data.bio} onChange={e => set("bio", e.target.value)} maxLength={300} />
        <p className="text-right text-[10px] font-bold text-slate-300">{data.bio.length}/300</p>
      </Field>
      <Field label="I am a..." icon={User}>
        <div className="grid grid-cols-2 gap-3">
          {[{ value: "freelancer", label: "Freelancer", sub: "Independent professional" }, { value: "business_owner", label: "Business Owner", sub: "Running a company/firm" }].map(opt => (
            <button key={opt.value} type="button" onClick={() => set("userType", opt.value)}
              className={`p-4 rounded-2xl border-2 text-left transition-all ${data.userType === opt.value ? "border-indigo-500 bg-indigo-50" : "border-slate-100 bg-slate-50/50 hover:border-slate-200"}`}>
...
const Page2 = ({ data, onChange }: { data: Page2Data; onChange: (d: Page2Data) => void }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <Field label="Company / Firm Name" icon={Building2}><input className={inputCls} placeholder="Acme Technologies" value={data.companyName} onChange={e => set("companyName", e.target.value)} /></Field>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <Field label="Industry" icon={Briefcase}><input className={inputCls} placeholder="e.g. IT Services, Design" value={data.industry} onChange={e => set("industry", e.target.value)} /></Field>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <Field label="Years in Business"><input className={inputCls} type="number" placeholder="3" value={data.yearsInBusiness} onChange={e => set("yearsInBusiness", e.target.value)} /></Field>
        <Field label="GSTIN (Optional)"><input className={inputCls} placeholder="22AAAAA0000A1Z5" value={data.gstin} onChange={e => set("gstin", e.target.value)} /></Field>
      </div>
      <Field label="Website" icon={Globe}><input className={inputCls} placeholder="https://yoursite.com" value={data.website} onChange={e => set("website", e.target.value)} /></Field>
      <div className="space-y-2">
        <label className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400"><MapPin size={11} /> Office Address</label>
...
const Page3 = ({ data, onChange }: { data: Page3Data; onChange: (d: Page3Data) => void }) => {
  ...
  const addService = () => { if (serviceInput.trim() && data.primaryServices.length < 6) { set("primaryServices", [...data.primaryServices, serviceInput.trim()]); setServiceInput(""); } };
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {data.primaryServices.map((s, i) => (
              <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-xl text-[11px] font-black">
                {s}<button type="button" onClick={() => set("primaryServices", data.primaryServices.filter((_, idx) => idx !== i))} className="text-indigo-400 hover:text-red-500">×</button>
              </span>
            ))}
          </div>
        )}
      ...
      <div className="space-y-3">
        <label className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400"><Target size={11} /> Current Priorities</label>
      ...
      <div className="space-y-3">
        <label className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400"><Star size={11} /> Goals</label>
      ...
      <Field label="Daily Business Scale" icon={Briefcase}>
        <textarea className={textareaCls} rows={2} placeholder="e.g. 3–4 client calls/day, 2 proposals/week..." value={data.dailyBusinessScale} onChange={e => set("dailyBusinessScale", e.target.value)} />
      </Field>
    </div>
  );
};
...
const Page4 = ({ data, onChange }: { data: Page4Data; onChange: (d: Page4Data) => void }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-3">
        <label className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400"><GraduationCap size={11} /> Education</label>
      ...
      <Field label="Hobbies & Interests" icon={Star}>
        <div className="flex gap-2">
          <input className={inputCls} placeholder="e.g. Open Source, Chess" value={hobInput} onChange={e => setHobInput(e.target.value)} onKeyDown={e => e.key === "Enter" && (e.preventDefault(), addHob())} />
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {data.hobbies.map((h, i) => (
              <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-600 rounded-xl text-[11px] font-black">
                {h}<button type="button" onClick={() => set("hobbies", data.hobbies.filter((_, idx) => idx !== i))} className="hover:text-red-500">×</button>
              </span>
            ))}
          </div>
        )}
      </Field>
      <Field label="Past Trade Highlights" icon={TrendingUp}>
        <textarea className={textareaCls} rows={3} placeholder="e.g. Closed ₹12L enterprise deal with XYZ in 2024..." value={data.pastTradeHighlights} onChange={e => set("pastTradeHighlights", e.target.value)} />
      </Field>
      <Field label="Current Targets This Quarter" icon={Target}>
        <textarea className={textareaCls} rows={2} placeholder="e.g. Close 3 enterprise clients, hit ₹8L MRR..." value={data.currentTargets} onChange={e => set("currentTargets", e.target.value)} />
      </Field>
    </div>
  );
};
...
const Page5 = ({ data, onChange }: { data: Page5Data; onChange: (d: Page5Data) => void }) => {
  const set = (k: keyof Page5Data, v: any) => onChange({ ...data, [k]: v });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-violet-50 border border-violet-100 rounded-2xl">
        <p className="text-[10px] font-black uppercase tracking-widest text-violet-600 mb-1">Why this matters</p>
        <p className="text-xs font-bold text-violet-700/80 leading-relaxed">
          These details will appear on every invoice you send — your clients will see exactly how to pay you. Keep them accurate.
        </p>
      </div>

      {/* Preferred Method */}
      <Field label="Preferred Payment Method" icon={CreditCard}>
        <div className="grid grid-cols-3 gap-3">
          {[
            { value: "upi",  label: "UPI",          icon: < MdOutlineMobileFriendly/> },
            { value: "bank", label: "Bank Transfer", icon: "🏦" },
            { value: "both", label: "Both",          icon: "✅" },
          ].map(opt => (
            <button key={opt.value} type="button" onClick={() => set("preferredMethod", opt.value)}
              className={`p-3 rounded-2xl border-2 text-center transition-all ${
                data.preferredMethod === opt.value
                  ? "border-violet-500 bg-violet-50"
                  : "border-slate-100 bg-slate-50/50 hover:border-slate-200"
              }`}>
              <p className="text-xl mb-1">{opt.icon}</p>
              <p className={`text-[11px] font-black ${data.preferredMethod === opt.value ? "text-violet-700" : "text-slate-600"}`}>{opt.label}</p>
            </button>
          ))}
        </div>
      </Field>
      ...
      {(data.preferredMethod === "upi" || data.preferredMethod === "both") && (
        <div className="space-y-4 p-5 bg-slate-50/50 rounded-2xl border border-slate-100">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
            <QrCode size={11} /> UPI Details
          </p>
          <Field label="UPI ID">
            <input className={inputCls} placeholder="yourname@ybl / yourname@okicici" value={data.upiId} onChange={e => set("upiId", e.target.value)} />
          </Field>
          <Field label="QR Code Image URL (optional)">
            <input className={inputCls} placeholder="https://cloudinary.com/your-qr.png" value={data.qrCode} onChange={e => set("qrCode", e.target.value)} />
          </Field>
          {data.qrCode && (
            <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-slate-100">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={data.qrCode} alt="QR Preview" className="w-16 h-16 object-contain rounded-lg" onError={e => (e.currentTarget.style.display = "none")} />
              <p className="text-[10px] font-bold text-slate-400">QR preview — clients will scan this to pay</p>
            </div>
          )}
        </div>
      )}
      ...
      {(data.preferredMethod === "bank" || data.preferredMethod === "both") && (
        <div className="space-y-4 p-5 bg-slate-50/50 rounded-2xl border border-slate-100">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1.5">
            <Landmark size={11} /> Bank Transfer Details
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Field label="Account Holder Name">
              <input className={inputCls} placeholder="Nevin Bali" value={data.accountName} onChange={e => set("accountName", e.target.value)} />
            </Field>
            <Field label="Account Number">
              <input className={inputCls} placeholder="XXXX XXXX XXXX" value={data.accountNo} onChange={e => set("accountNo", e.target.value)} />
            </Field>
            <Field label="IFSC Code">
              <input className={inputCls} placeholder="SBIN0001234" value={data.ifsc} onChange={e => set("ifsc", e.target.value.toUpperCase())} />
            </Field>
            <Field label="Bank Name">
              <input className={inputCls} placeholder="State Bank of India" value={data.bankName} onChange={e => set("bankName", e.target.value)} />
            </Field>
            <div className="sm:col-span-2">
              <Field label="Branch Name (Optional)">
                <input className={inputCls} placeholder="New Delhi Main Branch" value={data.branchName} onChange={e => set("branchName", e.target.value)} />
              </Field>
            </div>
          </div>
        </div>
      ...
      <p className="text-center text-[10px] font-bold text-slate-300 uppercase tracking-widest">
        You can skip this and add later from your profile page
      </p>
    </div>
  );
};
...
export default function ProfileSetupPage() {
  ...
  const [page5, setPage5] = useState<Page5Data>({ preferredMethod: "upi", upiId: "", accountName: "", accountNo: "", ifsc: "", bankName: "", branchName: "", qrCode: "" });
  ...
  const validateStep = () => {
    ...
    // Step 4 and 5 — optional fields, no hard validation
    return true;
  };

  const saveCurrentPage = async (pageNum: number) => {
    const pageData: Record<number, any> = { 1: page1, 2: page2, 3: page3, 4: page4, 5: page5 };
    const res = await fetch("/api/profile", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ pageNumber: pageNum, data: pageData[pageNum] }),
    });
  ...
  // Step 5 skip — save what's there and go to dashboard
  const handleSkipPayments = async () => {
    setSaving(true);
    try {
      await saveCurrentPage(5);
    } catch {
      // silently ignore — page5 is optional
    } finally {
      setSaving(false);
      toast.success("Profile complete! Welcome to Invoicer 🎉");
      router.push("/dashboard");
      ...
      <div className="relative z-10 max-w-3xl mx-auto px-4 py-10 md:py-16">
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-1.5">
                <motion.div animate={{ scale: isActive ? 1.08 : 1 }}
                  ...
                  <span className="hidden md:block text-[10px] font-black uppercase tracking-widest">{s.label}</span>
                </motion.div>
                {i < STEPS.length - 1 && <div className={`w-4 h-0.5 rounded-full ${isDone ? "bg-emerald-200" : "bg-slate-100"}`} />}
              </div>
            );
          })}
        </div>
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-slate-100 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 overflow-hidden">
          ...
          <div className="p-6 md:p-8 border-b border-slate-50 flex items-center gap-4">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                Step {step} of {TOTAL_STEPS}{step === 5 ? " · Optional" : ""}
              </p>
          ...
          <div className="p-6 md:p-8">
            <AnimatePresence mode="wait" custom={direction}>
              <motion.div key={step} custom={direction}
                ...
                {step === 5 && <Page5 data={page5} onChange={setPage5} />}
              </motion.div>
            </AnimatePresence>
          </div>
          ...
          <div className="px-6 md:px-8 pb-6 md:pb-8 flex gap-3">
            ...
            {/* Step 5: Skip button */}
            {step === 5 && (
              <button onClick={handleSkipPayments} disabled={saving}
                className="h-14 px-6 rounded-2xl border-2 border-slate-100 text-slate-400 font-black text-xs uppercase tracking-widest hover:border-slate-200 transition-all disabled:opacity-50">
                Skip for now
              </button>
            )}

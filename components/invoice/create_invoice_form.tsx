...
import {
  CalendarIcon, FileText, IndianRupee, Percent,
  Trash2, Send, Save, MapPin, Mail, Phone, User,
} from "lucide-react";
...
function BilledToClientDetails({ selectedClientDetails }: { selectedClientDetails: Client | null }) {
  if (!selectedClientDetails) {
    return (
      <div className="flex flex-col items-center justify-center py-8 gap-2 text-slate-300">
        <User size={32} strokeWidth={1.5} />
        <p className="text-[11px] font-black uppercase tracking-widest text-slate-400">No client selected</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-base font-black text-slate-900 tracking-tight">{clientName}</h3>
        {companyName && <p className="text-[11px] font-bold uppercase tracking-widest text-indigo-500 mt-0.5">{companyName}</p>}
      </div>
      <Separator className="bg-slate-100" />
      <div className="space-y-2.5">
        {(address || state || country) && (
          <div className="flex items-start gap-2.5">
            <div className="p-1.5 bg-slate-100 rounded-lg mt-0.5"><MapPin size={11} className="text-slate-500" /></div>
            <p className="text-[12px] text-slate-600 font-medium leading-relaxed">{[address, state, country, postal].filter(Boolean).join(", ")}</p>
          </div>
        )}
        {email && (
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 bg-slate-100 rounded-lg"><Mail size={11} className="text-slate-500" /></div>
            <p className="text-[12px] text-slate-600 font-medium">{email}</p>
          </div>
        )}
        {mobile && (
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 bg-slate-100 rounded-lg"><Phone size={11} className="text-slate-500" /></div>
            <p className="text-[12px] text-slate-600 font-medium">{countryTelephoneCode} {mobile}</p>
          </div>
        )}
      </div>
    </div>
...
const CreateInvoiceForm = () => {
  ...
  useEffect(() => { fetchClients(setClients); }, []);
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
              ...
              <FormField name="invoiceNumber" render={({ field }) => (
                <FormItem>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-full justify-between bg-slate-50 border-none rounded-xl font-black text-xs py-5">
                        {format(field.value, "PPP")}<CalendarIcon size={14} className="text-slate-400" />
                      </Button>
                    </PopoverTrigger>
            ...
            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-4">
              ...
              <FormField name="dueDate" render={({ field }) => (
                <FormItem className="flex flex-col">
                  <FormLabel className="text-[10px] font-black uppercase tracking-widest text-slate-400">Due Date</FormLabel>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button variant="outline" className="w-full justify-between bg-slate-50 border-none rounded-xl font-black text-xs py-5">
                        {field.value ? format(field.value, "PPP") : "Select Date"}<CalendarIcon size={14} className="text-slate-400" />
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="p-0 border-none shadow-2xl rounded-3xl overflow-hidden">
                      <Calendar mode="single" selected={field.value} onSelect={field.onChange} />
              ...
              <FormItem>
                <Select value={field.value} onValueChange={(val) => { field.onChange(val); setSelectedClient(clients.find((c) => c._id === val) || null); }}>
                  ...
                  <SelectContent className="rounded-2xl border-slate-100 shadow-2xl">
                    {clients.length === 0 ? (
                      <div className="py-4 text-center text-[11px] font-black uppercase tracking-widest text-slate-400">No clients found</div>
                    ) : (
                      clients.map((c) => (
                        <SelectItem key={c._id} value={c._id} className="font-bold py-3 uppercase text-[10px] tracking-widest">{c.clientName}</SelectItem>
                      ))
                    )}
                  </SelectContent>
                </Select>
                ...
                <div key={index} className="group grid grid-cols-1 md:grid-cols-12 gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm transition-all hover:border-indigo-200">
                  <div className="md:col-span-5 space-y-2">
                    ...
                    <FormField name={`items.${index}.name`} render={({ field }) => (
                      <Input placeholder="Service or Product Name" {...field} className="bg-slate-50 border-none rounded-xl h-12 font-bold" />
                    )} />
                  </div>
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-[9px] font-black uppercase text-slate-400 ml-1">Qty</label>
                    <FormField name={`items.${index}.quantity`} render={({ field }) => (
                      <Input type="number" {...field} className="bg-slate-50 border-none rounded-xl h-12 font-black italic" />
                    )} />
                  </div>
                  <div className="md:col-span-2 space-y-2">
                    <label className="text-[9px] font-black uppercase text-slate-400 ml-1">Rate</label>
                    <FormField name={`items.${index}.rate`} render={({ field }) => (
                      <Input type="number" {...field} className="bg-slate-50 border-none rounded-xl h-12 font-black italic" />
                    )} />
                  </div>
                  <div className="md:col-span-2 space-y-2">
            ...
            <div className="md:col-span-7 space-y-6">
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                ...
                <FormField name="description" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-[9px] font-black uppercase text-slate-400">Public Note</FormLabel>
                    <Textarea {...field} placeholder="Add a short description for the client..." className="min-h-[100px] bg-slate-50 border-none rounded-2xl p-4 font-medium" />
                  </FormItem>
                )} />
                <FormField name="termsAndConditions" render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-[9px] font-black uppercase text-slate-400">Terms & Conditions</FormLabel>
                    <Textarea {...field} placeholder="Payment policy, etc..." className="min-h-[100px] bg-slate-50 border-none rounded-2xl p-4 font-medium" />
                  </FormItem>
                )} />
              </div>
            </div>
              ...
              <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-6">
                    ...
                    <FormField name="discountPercent" render={({ field }) => (
                      <FormItem>
                        ...
                        <div className="relative">
                          <Input type="number" {...field} className="bg-slate-50 border-none rounded-xl pl-8 font-black italic" />
                ...
                <div className="space-y-3 pt-4">
                  <div className="flex justify-between text-[11px] font-black text-slate-400 uppercase tracking-widest">
                    <span>Sub Total</span><span className="text-slate-900 italic">₹{form.watch("subTotal").toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-[11px] font-black text-rose-400 uppercase tracking-widest">
                    <span>Discount</span><span className="italic">- ₹{form.watch("discountAmount").toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-[11px] font-black text-emerald-500 uppercase tracking-widest">
                    <span>Tax Amount</span><span className="italic">+ ₹{form.watch("taxAmount").toLocaleString()}</span>
                  </div>
